...
The error message indicates a conflict between the urllib3 library version and the underlying OpenSSL library version in your AWS Lambda environment. Here are ways to address this for the AWS user:
Solution 1: Downgrade urllib3 (if possible)
Compatibility Check: Check if downgrading urllib3 to a version compatible with OpenSSL 1.0.2k-fips is feasible for your use case. Refer to the urllib3 documentation for compatible versions.
Downgrade Command: If compatible, downgrade urllib3 within your Lambda deployment package using pip:
Bash
...
Caution: Downgrading might introduce security vulnerabilities in urllib3. Ensure the chosen version addresses those concerns.
Solution 2: Use an Alternative Library
Alternatives: Explore alternative libraries for URL parsing and unquoting that don't have the same OpenSSL dependency. Here are a few options:
requests: A popular HTTP library with built-in URL parsing capabilities.
urlsplit: A built-in Python module for basic URL splitting.
six.moves.urllib.parse (if using Python 2): Provides a compatibility layer for URL parsing across Python versions.
Installation and Usage: Install the chosen library using pip and update your code to use its functionalities for URL parsing and unquoting. Refer to the library's documentation for specific usage details.
Additional Considerations:
Higher Environments: Investigate why the Lambda function works in higher environments. It might be due to a different Python version or urllib3 installation. Consider standardizing the environment for consistency.
Testing: Thoroughly test any changes made to ensure your Lambda function continues to work as expected.
By implementing one of these solutions, the user should be able to resolve the import error and successfully run their Lambda function. Upgrading the Python version (if feasible) remains the most secure approach in the long run.
